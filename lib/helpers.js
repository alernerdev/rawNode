...
// detemine file content type based on filename extension
helpers.determineContentType = function determineContentType(assetName) {
    ...
    debug(`asset is ${assetName} determined content type ` + contentType);
    return contentType;
}

// get static (public) asset
helpers.getStaticAsset = function getStaticAsset(assetFileName, callback) {
    assetFileName = 
        typeof(assetFileName) == 'string' && assetFileName.length > 0 ? assetFileName : false;

    if (!assetFileName) {
        callback('a valid asset filename was not specified');
        return;
    }

    const fileNameToRead = path.join(__dirname, './../public/', assetFileName);
    debug(`reading asset ${fileNameToRead}`);
    fs.readFile(fileNameToRead, function(err, data) {
        if (!err && data)
            callback(false, data);
        else
